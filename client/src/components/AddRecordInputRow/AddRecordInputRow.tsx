      ...
      {props.type == 'file' &&
        <div className={`${styles.input} ${styles.file}`}>
          <FileUpload onFileChange={handleFileChange} className={styles.file}   {...(file && props.type == 'file' ? { file: file as File } : {})}>
            ...
            <div className={styles.fileText}> {file?.name  ?<span>{truncateText(file.name,40)} <span className={styles.size}> {formatFileSize(file.size)}</span> </span>:  props.placeholder} </div>
          </FileUpload>
        </div>
      }
      {props.type == 'custom' &&
